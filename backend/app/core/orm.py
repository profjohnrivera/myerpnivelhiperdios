...
class AsyncGraphSavepoint:
    ...
    def _clone_storage(self, storage_obj):
        ...
        elif hasattr(storage_obj, "cache"):
...
class Recordset:
    ...
    async def _check_row_rules(self, operation: str = "read") -> None:
        ...
        # Evitamos bloquear el propio motor técnico con RLS
    ...
    async def load_data(self) -> "Recordset":
        ...
        try:
            ...
            dirty_nodes = getattr(self._env.graph, "_dirty_nodes", set()) if self._env else set()
            ...
            for row in rows:
                ...
                for k, v in row.items():
                    if k == "x_ext" and v:
                        ...
                        if self._env and node_name not in dirty_nodes:
        ...
        async with AsyncGraphSavepoint(env):
            ...
            if ids:
                try:
                    ...
                    raise ValueError("❌ No se pueden eliminar los registros. Probablemente estén vinculados a otras operaciones (Integridad Referencial).")
...
class Model:
    ...
    async def search(
        ...
        if env and not env.su and str(getattr(env, "uid", "")) != "system":
            if model_name not in ["ir.rule", "ir.model", "ir.model.fields", "ir.model.access", "ir.sequence"]:
                try:
                    IrRuleModel = Registry.get_model("ir.rule")
                    user_domain = await IrRuleModel.get_domain(model_name, env.uid, operation="read")
                    if user_domain:
                        domain = ["&"] + user_domain + domain if domain else user_domain
                except Exception:
                    pass
